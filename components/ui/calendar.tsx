...
//   const reserved = reservation ? reservation.map((res) => startOfDay(new Date(res.date))) : [];

//   const freeDays: Date[] = [];
//   for (let d = new Date(today); d <= maxDate; d.setDate(d.getDate() + 1)) {
//     const isReserved = reservation?.some((res) => startOfDay(res.date).toString() === startOfDay(d).toString());
...
//   const disabledDaysReserv = reservation ? reservation.map((res) => ({ before: startOfDay(new Date(res.date)), after: startOfDay(new Date(res.date)) })) : [];

//   const disabledDaysAfterReserv =
//     pickUpDate && reservation
//       ? (() => {
//           const reservationDates = reservation.map((res) => startOfDay(new Date(res.date)));
...
//           const resDate = startOfDay(new Date(res.date));
//           return resDate > pickDateNormalize && resDate < date;
//         });
