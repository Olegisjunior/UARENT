...
export const ProductCart: React.FC<Props> = ({ id, name, description, imageUrl, price, fuel, transmision, seats }) => {
  const dispatch = useDispatch();
  const LikeDetails = useSelector((state: RootState) => state.like.cars);

  const handleLike = (id: number) => {
    const car = { id, name, imageUrl };
    dispatch(LikeDetails.some((car) => car.id === id) ? removeCar(car.id) : addCar(car));
  };
    ...
    <div className="item bg-white text-[#1A202C]  flex flex-col rounded-md  justify-between w-[305px] h-[390px]">
      <div className="flex justify-between">
        <div className="flex flex-col justify-center items-start px-5 pt-5">
          <Link href={`/car/${id}`}>
            <p className=" text-xl font-bold truncate hover:underline">{name}</p>
            <p className="text-[#90A3BF] font-normal truncate w-[180px] hover:underline">{description}</p>
          </Link>
        </div>

        <div className="px-5 pt-5">
          <Button variant="link" size={"icon"} className=" " onClick={() => handleLike(id)}>
            <Heart color={LikeDetails.some((el) => el.id === id) ? `#c53a3a` : `#90A3BF`} fill={LikeDetails.some((el) => el.id === id) ? `#c53a3a` : `none`} />
          </Button>
        </div>
      </div>
        ...
        <div className="icons flex justify-between items-center px-5 gap-2">
          <div className="flex justify-center items-center gap-2">
            ...
            <p className="text-[#90A3BF]">
              {fuel}
              {name.includes("Tesla") ? "kWh" : name.includes("Taycan") ? "kWh" : "L"}
            </p>
          </div>
          <div className="flex justify-center items-center gap-2">
